...
def backtest(args,is_fitting = True):
        ...
        if tick_time != time(20,59) and tick_time != time(8,59):
            if pos == 0:
                ...
                if len(detail['pos_price']) >= 1:
                    ...
                    if max_diff_1 >= 20 and max_diff_1 <= 35 and min_diff_1 == 0:
                        if sign == 1:
                            sign *= -1
                            changed_sign =1
                    elif row[1]['slope'] <= -10 and min_diff_1 <= -100:
                        if sign == -1:
                            sign *= -1
                            changed_sign =2
                    elif row[1]['grad'] >= 5 :
                        if sign == 1 and max_diff_1 >= 100:
                            sign *= -1
                            changed_sign =3
                        elif min_diff_1 >= -70 and min_diff_1 <= -30 and max_diff_1 <= 10:
                            if sign == 1:
                                sign *= -1
                                changed_sign = 4
                    
                    elif row[1]['grad'] <= -5 and min_diff_1 <= -80:
                        if sum(changed[-5:]) - sum(changed[-6:-1]) <= -70:
                            if sign == -1:
                                sign *= -1
                                changed_sign = 5
                    
                    elif row[1]['slope'] >0 and row[1]['slope'] <= 4:
                        if max_diff_1 >= 30 and max_diff_1 <= 100 and min_diff_1 >= -5:
                            if sign == -1:
                                sign *= -1
                                changed_sign = 6
                    elif max_diff_1 >= 10 and max_diff_1 <= 38:
                        if min_diff_1 <= -10 and min_diff_1 >= -38:
                            if total_diff[-1] <0:
                                sign *= -1
                                changed_sign = 7
    ...
    if is_fitting == True:
        ...
        detail_pd['max_diff'] = detail_pd['pos_price'] - detail_pd['pos_price'].rolling(15,min_periods=3).min()
        detail_pd['min_diff'] = detail_pd['pos_price'] - detail_pd['pos_price'].rolling(15,min_periods=3).max()
...
def get_data(path,start=20000,end=50000,period=1):
    ...
    h1 = data[(data['bid_volume']>=5*data['ask_volume'])&(data['grad']>=1)].index
    l1 =  data[(data['ask_volume']>=5*data['bid_volume'])&(data['grad']<=-1)].index
...
if __name__ == "__main__":
    ...
    """
    is_fitting = False
    input_path = os.path.dirname(__file__)+'/OI405_new_tick.csv'
    start = 0
    end = 900000
