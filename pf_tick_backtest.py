        ...
        if tick_time != time(20,59) and tick_time != time(8,59):
            if pos == 0:
                ...
                if len(detail['pos_price']) >= 1:
                    ...
                    # if sum(changed[-3:]) - sum(changed[-4:-1]) <= -10:
                    #     if min_diff_1 >= -40 and max_diff_1 <=60 and  sign == 1:
                    #         sign *= -1
                    #         changed_sign = 1
                    if min_diff_1 <= -200:
                        if sign == -1:
                            sign *= -1
                            changed_sign = 1
                    elif row[1]['slope'] <= -3 and row[1]['grad'] <= -3:
                        if sign == -1 and min_diff_1 <= -50:
                            sign *= -1
                            changed_sign = 2
                    elif max_diff_1 >= 100 and sum(changed[-3:]) - sum(changed[-4:-1]) <= -20:
                        if sign == 1 and sum(changed[-3:]) <= 30:
                            sign *= -1
                            changed_sign =3 
...
def get_data(path,start=20000,end=50000,period=1):
    ...
    h1 = data[(data['bid_volume']>=5*data['ask_volume'])&(data['grad']>=0.5)].index
    l1 =  data[(data['ask_volume']>=5*data['bid_volume'])&(data['grad']<=-0.5)].index
    data['sign']=0
    data.loc[h1,'sign'] = 1
    data.loc[l1,'sign'] = -1
...
if __name__ == "__main__":
    ...
    """
    is_fitting = False
    input_path = os.path.dirname(__file__)+'/PF405_new_tick.csv'
    start = 0
    end = 600000
