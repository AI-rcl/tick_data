    ...
    for row in tqdm(data.iterrows()):
        ...
        if is_trading(tick_time) == True:
            if pos == 0:
                ...
                if row[1]['max_diff']>=20 and row[1]['min_diff']>=-1:
                    if sign == 1:
                        sign *= -1
                        changed_sign = 1
...
def get_data(path,start=20000,end=50000,period=1):
    ...
    h1 = data[(data['last_price']<=data['bid_price'])&(data['bid_volume']>3*data['ask_volume'])&(data['slope']>=0.3)].index
    l1 =  data[(data['ask_volume']>3*data['bid_volume'])&(data['slope']<=-0.3)&(data['grad']>0)].index
    data['sign']=0
    data.loc[h1,'sign'] = 1
    data.loc[l1,'sign'] = -1
...
if __name__ == "__main__":
    ...
    """
    is_fitting = False
    input_path = "D:/Code/jupyter_project/data_analysis/bar_analysis/bar_backtest/ma数据分析/MA405_tick_new.csv"
    start = 0
    end = 800000
