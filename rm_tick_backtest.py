...
def get_data(path,start=20000,end=50000,period=1):
    ...
    h1 = data[(data['bid_volume']>=3*data['ask_volume'])&(data['slope']>= 0.2)&(data['grad']< 0)].index
    l1 =  data[(data['ask_volume']>3*data['bid_volume'])&(data['slope']<= -0.2)&(data['grad']>=0)].index
    data['sign']=0
    data.loc[h1,'sign'] = 1
    data.loc[l1,'sign'] = -1
...
if __name__ == "__main__":
    ...
    """
    is_fitting = False
    input_path = os.path.dirname(__file__)+'/rm405_new_tick.csv'
    start = 0
    end = 1200000
