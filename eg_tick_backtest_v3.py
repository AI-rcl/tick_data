...
def is_trading(trade_time):
    start_1 = time(8,0)
    end_1 = time(9,0)

    start_2 = time(20,0)
    end_2 = time(21,0)
    if trade_time >= start_1 and trade_time < end_1:
        return False
    elif trade_time >= start_2 and trade_time < end_2:
        return False
    return True
...
def backtest(args,is_fitting = True):
        ...
        if is_trading(tick_time) == True:
            if pos == 0:
                if len(detail['pos_price']) >= 1:
                    max_diff_1 = row[1]['ask_price'] - min(detail['pos_price'][-19:])
                    min_diff_1 = row[1]['bid_price'] - max(detail['pos_price'][-19:])
                    max_diff_2 = detail['pos_price'][-1] - min(detail['pos_price'][-20:])
                    min_diff_2 = detail['pos_price'][-1] - max(detail['pos_price'][-20:])
                    if max_diff_1 <=30 and min_diff_1 >= 0:
                        if sign == -1:
                            sign *= -1
                            changed_sign =1
                    elif max_diff_1 >= 40 and min_diff_1 <0 and min_diff_2 ==0:
                        if sign == 1:
                            sign *= -1
                            changed_sign =2
                    ...
                    elif min_diff_1 <= -35 and min_diff_1 >=-70:
                        if row[1]['grad'] <= -3:
                            if sign == -1:
                                sign *= -1
                                changed_sign = 5
    ...
    if is_fitting == True:
        ...
        detail_pd['max_diff'] = detail_pd['pos_price'] - detail_pd['pos_price'].rolling(20,min_periods=5).min()
        detail_pd['min_diff'] = detail_pd['pos_price'] - detail_pd['pos_price'].rolling(20,min_periods=5).max()
...
def get_data(path,start=20000,end=50000,period=1):
    ...
    data['max_diff'] = data['last_price'] - data['min_1']
    data['min_diff'] = data['last_price'] - data['max_1']

    h1 = data[(data['last_price']<=data['bid_price'])&(data['bid_volume']>6*data['ask_volume'])&(data['slope']>=0.5)].index
    l1 =  data[(data['ask_volume']>4*data['bid_volume'])&(data['slope']<=-0.5)].index
...
def get_param(data):
    ...
    data['max_1'] = data['high'].rolling(180).max()
    data['min_1'] = data['low'].rolling(180).min()
