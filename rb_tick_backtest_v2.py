...
def backtest(args,is_fitting = True):
    ...
    for row in tqdm(data.iterrows()):
        ...
        if tick_time != time(20,59) and tick_time != time(8,59):
            if pos == 0:
                if row[1]['grad'] >= 5 and row[1]['slope']<=1:
                    if sign == 1:
                        sign *= -1
                        changed_sign =1
...
def get_data(path,start=20000,end=50000,period=1):
    ...
    l1 =  data[(data['ask_volume']>5*data['bid_volume'])&(data['slope']<= 0.5)&(data['grad']>0)].index
    data['sign']=0
    data.loc[h1,'sign'] = 1
    data.loc[l1,'sign'] = -1
...
if __name__ == "__main__":
    ...
    """
    is_fitting = False
    input_path = 'D:/Code/python_project/bar_analysis/rb_bar/rb2405_new_tick.csv'
    start = 0
    end = 1100000
